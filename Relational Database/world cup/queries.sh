...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games AS w ON w.winner_id = team_id FULL JOIN games AS o ON o.opponent_id = team_id WHERE (w.year = 2014 AND w.round = 'Eighth-Final') OR (o.year = 2014 AND o.round = 'Eighth-Final') ORDER BY name;")"
